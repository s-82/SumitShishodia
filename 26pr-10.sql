create database 26june;
use 26june;
create table employee(eid int,name varchar(49),salary int,doj DATE);
...
select * from users;
alter table users CHANGE eid id int;
...
salary in (49000,63000,51000);
select * from employee WHERE
name in (select name from employee where name like 'a%');
